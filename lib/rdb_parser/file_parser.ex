defmodule RdbParser.FileParser do
  ...
  representing the type, and a second argument that is a 3-tuple. The 3-tuple
  is typically {key::binary, value::term, metadata::Keyword.t}, although for some
  metadata fields, the fields do not strictly match up.
  """
  @spec parse_file(String.t(), (field_type(), {binary(), term(), Keyword.t | nil} -> any())) :: :ok | {:error, binary()}
  ...
  def parse(<<@rdb_opcode_resizedb, rest :: binary>>, func) do
    case parse_length(rest) do
      ...
      {len, rest} ->
        ...
        func.(:dbsize, len)
        func.(:expire_dbsize, expirelen)
        parse(rest, func)
    end
  end

  def parse(<<@rdb_opcode_expiretime, expiration_time :: little-unsigned-integer-size(32), @rdb_type_string, rest :: binary>>, func) do
    with {key, rest} <- parse_string(rest),
         {value, rest} <- parse_string(rest)
    do
  ...
  def parse(<<@rdb_opcode_expiretime_ms, expiration_time :: little-unsigned-integer-size(64), @rdb_type_string, rest :: binary>>, func) do
    with {key, rest} <- parse_string(rest),
         {value, rest} <- parse_string(rest)
    do
  ...
  def parse_length(<<  2 :: size(2), 0::size(6), len :: little-unsigned-integer-size(32), rest :: binary>>), do: {len, rest}
  def parse_length(<<  3 :: size(2), 0::size(6), len :: little-unsigned-integer-size(64), rest :: binary>>), do: {len, rest}
